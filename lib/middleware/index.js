...
module.exports = options => {
  ...
  const isValidInt = value => isInt.test(`${value}`);
  const toInt = (value, defaultValue) => isValidInt(value) ? parseInt(value) : defaultValue;

  const validateAndFind = (req, res) => validateRequest(req, res, opts.find.rules).then(() => {
    const body = opts.find.getReqBody(req, res);
    const find = crud.find(opts.find.getConditions(body));
    const controls = opts.find.getControls(opts.find.useControlsFromQuery ? req.query : body);
    if (opts.find.omitControls && !isValidInt(controls.skip) && !isValidInt(controls.limit)) {
      return find.exec();
    }
    return find
      .skip(toInt(controls.skip, 0))
      .limit(toInt(controls.limit, 20))
      .exec();
  });
